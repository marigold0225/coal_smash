...
ParticleData calculate_info_alpha_twobody(const ParticleData &d1, const ParticleData &d2,
    ...
    alpha.probability = 1.0 / 16 * 8 * exp(-diff_dr * diff_dr / rms / rms - diff_dp * diff_dp * rms * rms / hbar2);
...
ParticleData calculate_info_alpha_fourbody(const ParticleData &p1, const ParticleData &p2,
                                           ...
                                           const config_in &config_input, std::vector<double> &alpha_mix_spv) {
    double sig1                  = config_input.rms;
    double sig2                  = config_input.rms;
    double sig3                  = config_input.rms;
...
calculate_diff_fourbody(const ParticleData &p1, const ParticleData &p2,
    ...
    double dx_p1p2n1n2 = (p1.mass * p1.mass + p2.mass * p2.mass +
                          n1.mass * n1.mass - (p1.mass + p2.mass + n1.mass) * n2.x) /
                         (p1.mass + p2.mass + n1.mass) * sqrt(3.0 / 4.0);
    double dy_p1p2n1n2 = (p1.mass * p1.mass + p2.mass * p2.mass +
                          n1.mass * n1.mass - (p1.mass + p2.mass + n1.mass) * n2.y) /
                         (p1.mass + p2.mass + n1.mass) * sqrt(3.0 / 4.0);
    double dz_p1p2n1n2 = (p1.mass * p1.mass + p2.mass * p2.mass +
                          n1.mass * n1.mass - (p1.mass + p2.mass + n1.mass) * n2.z) /
                         (p1.mass + p2.mass + n1.mass) * sqrt(3.0 / 4.0);
...
void calculate_alpha_fourbody_batch(const std::vector<ParticleData> &protons, const std::vector<ParticleData> &neutrons,
    ...
    std::priority_queue<ParticleData, std::vector<ParticleData>, decltype(compare)> alpha_queue(
            compare);
        ...
        for (size_t j = i + 1; j < protons_fraction.size(); j++) {
            for (size_t k = 0; k < neutrons_fraction.size(); k++) {
                for (size_t l = k + 1; l < neutrons_fraction.size(); l++) {
                    ...
                    batch_alpha += alpha_particle.probability / mixEvents * factor;
                    if (alpha_particle.probability > 0) {
                        alpha_queue.push(alpha_particle);
                    }
                }
            }
        }
    }
    int expected_deutrons = static_cast<int>(batch_alpha);
    double fraction       = batch_alpha - expected_deutrons;
    if (dis(gen) < fraction) {
        expected_deutrons++;
    }
    while (!alpha_queue.empty() && expected_deutrons > 0) {
        alpha.push_back(alpha_queue.top());
        alpha_queue.pop();
        expected_deutrons--;
    }
...
void calculate_alpha_fourbody(const std::string &protonFile, const std::string &neutronFile,
    ...
    std::ofstream alphaFileOut;
    if (!std::filesystem::exists(alphaFile)) {
        alphaFileOut.open(alphaFile, std::ios::out);
    } else {
        alphaFileOut.open(alphaFile, std::ios::app);
    }
    ...
    for (const auto &[batchNumber, batchData]: protonBatches) {
        ...
        total_alpha += batch_number_alpha;
        total_batches++;
        alpha.insert(alpha.end(), batch_alpha.begin(), batch_alpha.end());
        if (alphaFileOut.is_open()) {
            for (const auto &particle: batch_alpha) {
                alphaFileOut << particle.freeze_out_time << " " << particle.x << " " << particle.y << " " << particle.z << " "
                             << particle.px << " " << particle.py << " " << particle.pz << " " << particle.p0 << " "
                             << particle.mass << " " << particle.probability << "\n";
            }
        }
        std::cout << "average number of alpha per batch:" << batch_number_alpha << std::endl;
    }
    alphaFileOut.close();
    ...
    output_d_mix_spv(alphaMixSpv, alphaMixPtv, "tem/alpha_mix_spv.dat", total_batches);
}
void calculate_alpha_twobody_batch(const std::vector<ParticleData> &deutrons, const config_in &config_input,
    ...
    int expected_alpha = static_cast<int>(batch_alpha);
    double fraction    = batch_alpha - expected_alpha;
    if (dis(gen) < fraction) {
        expected_alpha++;
    }
    while (!alpha_queue.empty() && expected_alpha > 0) {
        alpha.push_back(alpha_queue.top());
        alpha_queue.pop();
        expected_alpha--;
    }
